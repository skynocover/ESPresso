...
The firmware provides a device-local Pomodoro timer that is entered by tilting the board into landscape orientation. It takes over the display as a fullscreen countdown, supports pause/resume by tap, runs entirely on the device clock without depending on the Mac agent, and signals completion using the screen and backlight only. State is volatile and never persisted.
## Requirements
